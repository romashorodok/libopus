...
use crate::{
  ebml::EbmlHeader,
  elements::{
    Audio, Cluster, Colour, Info, Lacing, Seek, SeekHead, SimpleBlock,
...
impl MkvMuxer {
  pub fn matroska() -> MkvMuxer {
    MkvMuxer {
      ...
      seek_head: SeekHead { positions: Vec::new() },
      info: None,
      tracks: None,
      blocks: Vec::new(),
      blocks_len: 0,
  ...
  pub fn write_segment_header(
    &mut self, buf: &mut Vec<u8>, _size: usize,
  ) -> Result<()> {
    let mut origin = (buf).as_ptr() as usize;
...
impl Muxer for MkvMuxer {
  ...
  fn write_header<W: Write>(&mut self, out: &mut Writer<W>) -> Result<()> {
    ...
    let info_seek = Seek { id: [0x15, 0x49, 0xA9, 0x66], position: 0 };
    let tracks_seek = Seek { id: [0x16, 0x54, 0xAE, 0x6B], position: 0 };
    let cluster_seek = Seek { id: [0x1F, 0x43, 0xB6, 0x75], position: 0 };
    ...
    self.seek_head.positions[0].position =
      self.seek_head.size(0x114D9B74) as u64;
  ...
  fn write_packet<W: Write>(
    &mut self, out: &mut Writer<W>, pkt: Arc<Packet>,
  ) -> Result<()> {
    ...
    // print!("empty tiemstamp {:?}", self.timestamp.is_none());
    ...
    if self.timestamp.is_none() {
      self.timestamp = Some(pkt.t.pts.or(pkt.t.dts).unwrap_or(0) as u64);
    }

    if pkt.is_key || self.blocks_len >= 5242880 {
      {
        let simple_blocks: Vec<&[u8]> =
          self.blocks.iter().map(|v| &v[..]).collect();
...
#[allow(dead_code)]
fn offset<'a>(
  original: &(&'a [u8], usize), subslice: &(&'a [u8], usize),
) -> usize {
...
#[allow(dead_code)]
fn gen_mkv_prefix<'b>(
  input: (&'b mut [u8], usize), header: &EbmlHeader, seek_head: &SeekHead,
